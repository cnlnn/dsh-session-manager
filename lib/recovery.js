...
import { readStored, rebuildProjection, resolveStoredPreset, storedHeaders } from './compat.js'
...
function installModelComposition(ctx, agentCtx, agent = agentCtx?.agent) {
...
async function resumeComposition(ctx, meta, events) {
  const presets = service(ctx, 'agentPresets')
  const presetId = resolveStoredPreset(meta, events)
  ...
  return {
    ...
    setup: async (agentCtx, agent) => {
      installModelComposition(ctx, agentCtx, agent)
...
async function revisionFor(persistence, sessionId) {
  if (typeof persistence.stat === 'function') return (await persistence.stat(sessionId))?.revision
...
async function inspectForResume(ctx, persistence, sessionId, fallback) {
  ...
  if (typeof query?.observeSession === 'function') {
    ...
    try {
      if (observation.header.cwd === undefined) throw new Error('session has no workspace')
  ...
  if (typeof ctx?.get === 'function' && inspected.meta.cwd === undefined) throw new Error('session has no workspace')
...
async function reconcileProjection(ctx, meta, lastSeq, logger) {
  ...
  try {
    await rebuildProjection(ctx, meta.id)
...
export class SessionRecoveryCoordinator {
  ...
  start({ immediate = false } = {}) {
    ...
    this.started = true
    if (immediate) return this.reconcile()
    this.schedule()
  }
  ...
  async reconcileOnce() {
    ...
    if (typeof persistence?.list !== 'function'
      || (typeof persistence?.readFrom !== 'function' && typeof persistence?.open !== 'function')) {
    ...
    try {
      const headers = await storedHeaders(persistence)
      for (const meta of headers) {
        ...
        let initialRevision
        try {
          raw = await readStored(persistence, meta.id)
  ...
  async recoverGoal(sessionId, expected, facts, result, attemptKey, initialRevision, sourceMeta) {
    ...
    try {
      ...
      const latest = await readStored(persistence, sessionId)
    ...
    try {
      ...
      const final = await readStored(persistence, sessionId)
