		...
		function SessionManagerPluginCard({ ctx }) {
			...
			const load = React.useCallback(async () => {
				try {
					...
					const next = row.value?.showSidebarTrash !== false;
					setValue(next);
					setSaved(next);
			...
			const save = async () => {
				...
				try {
					const response = await api.settings.mutate({
						...
						ops: [{ op: "set", path: ["showSidebarTrash"], value }],
						expectedRevision: revision,
					});
					if (!response.result.ok) throw new Error(response.result.error.message);
					setRevision(response.result.value.revision);
					setSaved(response.result.value.value.showSidebarTrash !== false);
					signalChanged();
				} catch (error) {
					setFailure(error instanceof Error ? error.message : String(error));
				} finally {
				...
				open ? h("div", { className: "dsm-plugin-body" },
					...
					h("div", { className: "dsm-plugin-field" },
						h("label", { className: "dsm-plugin-toggle" },
							h("input", {
								type: "checkbox", checked: value, disabled: busy || !writable,
