...
`;
		...
		function SessionRow({ item, onTrash }) {
			return h("div", { className: "dsm-row" },
				...
				h("div", { className: "dsm-actions" },
					...
					h("button", { className: "dsm-action dsm-danger", type: "button", disabled: item.attached, onClick: () => onTrash(item) }, "移到回收站"),
				),
			);
		}

		function MenuTrashController({ refreshRuntime }) {
			...
			React.useEffect(() => {
				const confirm = event => {
					setFailure("");
					setAction({ kind: "trash", item: event.detail });
				};
				const report = event => {
					setAction({ kind: "trash", item: { id: "", title: "会话" } });
					setFailure(event.detail);
				};
				window.addEventListener("dsm:confirm-trash", confirm);
				window.addEventListener("dsm:trash-error", report);
				return () => {
					window.removeEventListener("dsm:confirm-trash", confirm);
					window.removeEventListener("dsm:trash-error", report);
				};
			}, []);
			...
			const confirm = async () => {
				...
				setFailure("");
				try {
					await api("trash", { sessionId: action.item.id });
		...
		function installSessionMenuDelete() {
			...
			const addDeleteItem = () => {
				...
				for (const menu of menus) {
					if (menu.querySelector('[data-dsm-session-delete]') !== null) return;
					...
					button.dataset.dsmSessionDelete = "";
					button.classList.add("dsm-menu-delete");
					label.textContent = "删除会话";
					button.addEventListener("click", event => {
						...
						window.setTimeout(() => {
							if (id.length === 0) {
								window.dispatchEvent(new CustomEvent("dsm:trash-error", { detail: "无法识别该会话，请刷新页面后重试" }));
								return;
							}
							window.dispatchEvent(new CustomEvent("dsm:confirm-trash", { detail: { id, title } }));
						}, 0);
					});
					archive.parentElement.after(wrapper);
					return;
				}
			};
			...
			const schedule = (force = false) => {
				...
				if (scheduled) return;
				scheduled = true;
				window.setTimeout(addDeleteItem, 0);
			};
			const onClick = event => {
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				root.render(h(MenuTrashController, { refreshRuntime }));
				const disposeMenu = installSessionMenuDelete();
				return () => {
					disposeMenu();
					root.unmount();
					container.remove();
