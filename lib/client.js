		...
		function SessionRow({ item, onTrash, onDelete }) {
			...
			return h("div", { className: "dsm-row" },
				...
				h("div", { className: "dsm-actions" },
					...tags,
					h("button", { className: "dsm-action", type: "button", disabled: item.running, onClick: () => onTrash(item) }, "移到回收站"),
					h("button", { className: "dsm-action dsm-danger", type: "button", disabled: item.running, onClick: () => onDelete(item) }, "永久删除"),
				),
			);
		}
