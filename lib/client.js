window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { IconCloseOutline16, IconDownloadOutline16, IconRefreshOutline16, IconTrashOutline16 } = require("@deepseek-ai/dsh-client-ui-primitives");
...
`;
		...
		function SessionRow({ item, onTrash, onDelete }) {
			...
			return h("div", { className: "dsm-row" },
				...
				h("div", { className: "dsm-actions dsm-session-actions" },
					h("button", { className: "dsm-icon-button dsm-row-action", type: "button", title: "移到回收站", "aria-label": `将“${item.title}”移到回收站`, disabled: item.running, onClick: () => onTrash(item) }, h(IconDownloadOutline16, { size: 16 })),
		...
		function TrashRow({ item, busy, onRestore, onPurge }) {
			return h("div", { className: "dsm-row" },
				...
				h("div", { className: "dsm-actions dsm-session-actions" },
					h("button", { className: "dsm-icon-button dsm-row-action", type: "button", title: "恢复", "aria-label": `恢复“${item.title}”`, disabled: busy, onClick: () => onRestore(item) }, h(IconRefreshOutline16, { size: 16 })),
		...
		function TrashSidebarAction({ wide, refreshRuntime }) {
			...
			return h(React.Fragment, null,
				h("button", {
					...
					h("span", { className: "dsm-icon-source dsm-move-icon-source", "aria-hidden": "true" }, h(IconDownloadOutline16, { size: 16 })),
